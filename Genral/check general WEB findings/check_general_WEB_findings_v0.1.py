...
# check_xframe // to add this feature in next version
# check_banner // to add this feature in next version
# check_cookie_httponly // to add this feature in next version
# check_cookie_secure // to add this feature in next version
# check_http_methods // to add this feature in next version
...
#########################################################################\n\
    ...
    check_general_WEB_finding.py 0.1\n\
    (0.1 17/09/2017 09:56 ITC)\n\
\n\
\n\
...
def init():
	...
	REGEX_WEAK_CIPHER = r'm([0-9]|[1-9][0-9]|((1[0-1][0-9])|(12[0-7])))[^0-9a-zA-Z].*bits[^\r\n$]'
...
def check_SSL_protocol_version(url):
	if validate_https_scheme(url):
		# print "[!] Checking support protocol"
...
def check_sweet32(url):
	if validate_https_scheme(url):
		...
		(output, err) = p.communicate()
		vuln_stmt = "[+] SWEET32\t\tVulnerable" + " [" + gettime()  + "]"
...
def check_heartbleed(url):
	if validate_https_scheme(url):
		...
		(output, err) = p.communicate()
		vuln_stmt = "[+] HeartBleed\t\tVulnerable" + " [" + gettime()  + "]"
...
def check_ccs(url):
	if validate_https_scheme(url):
		...
		(output, err) = p.communicate()
		vuln_stmt = "[+] CCS Injection\tVulnerable" + " [" + gettime()  + "]"
...
def check_crime(url):
	if validate_https_scheme(url):
		...
		(output, err) = p.communicate()
		vuln_stmt = "[+] CRIME\t\tVulnerable" + " [" + gettime()  + "]"
...
def check_beast(url):
	if validate_https_scheme(url):
		...
		(output, err) = p.communicate()
		vuln_stmt = "[+] BEAST\t\tVulnerable" + " [" + gettime()  + "]"
...
def check_breach(url):
	if validate_https_scheme(url):
		...
		(output, err) = p.communicate()
		vuln_stmt = "[+] BREACH\t\tVulnerable" + " [" + gettime()  + "]"
...
def check_poodle(url):
	if validate_https_scheme(url):
		...
		(output, err) = p.communicate()
		vuln_stmt = "[+] POODLE\t\tVulnerable" + " [" + gettime()  + "]"
...
def check_freak(url):
	if validate_https_scheme(url):
		...
		(output, err) = p.communicate()
		vuln_stmt = "[+] FREAK\t\tVulnerable" + " [" + gettime()  + "]"
...
def check_logjam(url):
	if validate_https_scheme(url):
		...
		(output, err) = p.communicate()
		vuln_stmt = "[+] LOGJAM\t\tVulnerable" + " [" + gettime()  + "]"
...
def check_drown(url):
	if validate_https_scheme(url):
		...
		(output, err) = p.communicate()
		vuln_stmt = "[+] DROWN\t\tVulnerable" + " [" + gettime()  + "]"
...
def check_rc4(url):
	if validate_https_scheme(url):
		...
		(output, err) = p.communicate()
		vuln_stmt = "[+] RC4\t\t\tVulnerable" + " [" + gettime()  + "]"
...
def check_weak_cipher(url):
	if validate_https_scheme(url):
		...
		p = subprocess.Popen("sslscan --no-renegotiation --no-compression --no-heartbleed --no-fallback --no-check-certificate " + str(url), stdout=subprocess.PIPE, shell=True)
		(output, err) = p.communicate()
		vuln_stmt = "[+] Weak cipher\t\tVulnerable" + " [" + gettime()  + "]"
...
def cert_info(url):
	if validate_https_scheme(url):
		...
		if not has_cert:
			...
			if is_wrong_host:
				print "   [+] Certificate does not match supplied URI"
			...
			if is_weak_algo:
				print "   [+] Certificate weak hash algorithm"
...
if __name__ == "__main__":
	...
	if args.output_dir:
		if os.path.isdir(str(args.output_dir)):
			...
			CSV_DIR = PROOF_DIR
			print "[!] All proves and CSV result will be stored at '" + PROOF_DIR + "'"
		else:
			print "[!] Argument -oA should come with existing full directory path"
			exit(0)
	elif args.output_csvdir:
		if os.path.isdir(str(args.output_csvdir)):
			TO_COLLECT_CSV = True
			CSV_DIR = (str(args.output_csvdir) + "/").replace("//", "/")
			print "[!] Only CSV result will be stored at '" + CSV_DIR + "'"
		else:
			print "[!] Argument -oA should come with existing full directory path"
			exit(0)
	...
	with open(fname) as f:
		for line in f:
			...
			if check_connection_to_target(line) == False:
				print "[!] Connection error, skipping this host"
			else:
				RESULT_DIC[str(line)] = {}
				if args.check_all:
					cert_info(line)
